"""
Generate manuscript Word document from markdown chapters.

Default output targets the v14 manuscript, but a custom output path can be
...
DEFAULT_OUTPUT = BASE / "manuscript/word/数据要素利用与资产定价效率v14.docx"
...
INLINE_VAR_RE = re.compile(
    r'\$[^$]+\$'                        # $inline math$
    r'|DU_\{?llm[_\\]?lenstd\}?'       # DU_llm_lenstd
    r'|DU_\{?kw[_\\]?strict\}?'        # DU_kw_strict
    r'|DU_\{?kw[_\\]?resid\}?'         # DU_kw_resid
    ...
    r'|SYNCH'                           # SYNCH
    r'|GenericNarr'                     # GenericNarr
    r'|WashGap'                         # WashGap
    r'|BroadShallow'                    # BroadShallow
    r'|llm_binary'                      # llm_binary
    r'|llm_score'                       # llm_score
    r'|ln_total_chars'                  # ln_total_chars
    r'|ln_mda_chars'                    # ln_mda_chars
...
def add_omml_formula(doc, latex_str):
    ...
    if parser.eq_num:
        ...
        tabs_old = pPr.find(qn("w:tabs"))
        if tabs_old is not None:
            pPr.remove(tabs_old)
        jc = pPr.find(qn("w:jc"))
        if jc is not None:
            pPr.remove(jc)
        ...
        tabs.append(tab_r)
        # tabs must appear before spacing/jc in pPr child order for schema validation
        pPr.insert(0, tabs)
        pPr.append(jc)
...
def _set_cell_borders(cell, top=None, bottom=None, left=None, right=None):
    ...
    # Use start/end instead of left/right to satisfy current OOXML schema validation.
    for edge_name, props in [("top", top), ("start", left), ("bottom", bottom), ("end", right)]:
        if props:
...
def _clear_table_borders(table):
    ...
    # Set empty borders. Insert before tblLook when present because tblBorders
    # must appear before tblLook in tblPr child order.
    ...
    if tblLook is not None:
        tblPr.insert(list(tblPr).index(tblLook), borders)
    else:
