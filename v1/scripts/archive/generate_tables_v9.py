...
# Extract intercepts for all baseline models
m0_const = get_intercept(m0, reg, 'PriceDelay', ['DU_kw'])
m1_const = get_intercept(m1, reg, 'PriceDelay', ['DU_kw'] + controls)
m2_const = get_intercept(m2, reg, 'PriceDelay', ['DU_kw_ln'] + controls)
m3_const = get_intercept(m3, reg, 'PriceDelay', ['DU_sub_ln'] + controls)
m4_const = get_intercept(m4, reg_ind, 'PriceDelay', ['DU_kw'] + controls)
m5_const = get_intercept(m5, reg_synch, 'SYNCH', ['DU_kw'] + controls)
m6_const = get_intercept(m6, reg, 'PriceDelay', ['DU_kw', 'FinAsset'] + controls)
all_const = [m0_const, m1_const, m2_const, m3_const, m4_const, m5_const, m6_const]
print(f"  Baseline done. m0={m0_main['DU_kw'][0]:.4f} N={m0_N}, m1={m1_coefs['DU_kw'][0]:.4f} N={m1_N}")
print(f"  Intercepts: {[f'{c[0]:.3f}' if c[0] is not None else 'NA' for c in all_const]}")
...
# Constant row
const_cr = [P_var('Constant')]
const_sr = [P('', cell_left)]
for i in range(7):
    c, s, sig = all_const[i]
    if c is not None:
        const_cr.append(P_coef(c, sig))
        const_sr.append(P_se(s))
    else:
        const_cr.append(P('', cell_style))
        const_sr.append(P('', cell_style))
t2_data.append(const_cr)
t2_data.append(const_sr)
