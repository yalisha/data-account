"""
生成朱康(2025)会计研究格式的全部表格PDF — v9.2
v9.2修复:
  - 表3: 全部用Stata reghdfe重新验证, 替换为8列新结构
    (1)Ind*Year FE (2)Prov*Year FE (3)行业均值调整 (4)剔除2024
    (5)剔除IT (6)仅主板 (7)PSM (8)双向聚类SE
  - 删除伪Heckman两阶段(统计上不成立)
  - 新增: Ind*Year FE / Prov*Year FE / 行业均值调整 / 双向聚类
v9.1修复:
...
pdf_path = f"{BASE}/results/v9_tables/regression_tables_v9_2.pdf"
...
rob_dvs = ['PriceDelay']*8

rob_keys = ['indyear','provyear','indadj','no2024','noit','mainboard','psm','twoway']
rob_models = [rob_stata[k] for k in rob_keys]

# Variable display names per column
rob_var_names = ['DU_kw','DU_kw','DU_kw_adj','DU_kw','DU_kw','DU_kw','DU_kw','DU_kw']

# FE structure rows
rob_fe_firm =    ['YES','YES','YES','YES','YES','YES','YES','YES']
rob_fe_year =    ['NO', 'NO', 'YES','YES','YES','YES','YES','YES']
rob_fe_indyear = ['YES','NO', 'NO', 'NO', 'NO', 'NO', 'NO', 'NO']
rob_fe_provyear =['NO', 'YES','NO', 'NO', 'NO', 'NO', 'NO', 'NO']
...
story.append(Paragraph(
    ...
    "在匹配样本上以连续DU_kw回归。"
    "模型(8)使用企业和年份双向聚类标准误。", note_style))
story.append(PageBreak())

# ============================================================
# 表4: 内生性检验 (v9.3: Panel A第一阶段 + Panel B第二阶段, Stata验证)
# ============================================================
print("  Table 4...")
...
# First-stage results from Stata reghdfe verification
# Format: (IV_name, coef, se, sig, N, R2)
fs_stata = {
    'peer':   ('DU_kw_peer', 0.5970, 0.0367, '***', 43779, 0.844),
    'bartik': ('bartik_iv',  0.2873, 0.0367, '***', 43058, 0.841),
    'lag':    ('peer_lag',   0.6018, 0.0444, '***', 37340, 0.836),
}

# Second-stage data (from pyfixest 2SLS, Stata-verified direction/magnitude)
iv_models_data = [
...
# DV row
t4a_data.append([P('', cell_left)] + [P('<i>DU_kw</i>', ParagraphStyle('dv', fontName='Times-Italic',
                fontSize=8, leading=10, alignment=TA_CENTER)),
                P('<i>DU_kw</i>', ParagraphStyle('dv2', fontName='Times-Italic',
                fontSize=8, leading=10, alignment=TA_CENTER)),
                P('<i>DU_kw_lag</i>', ParagraphStyle('dv3', fontName='Times-Italic',
                fontSize=8, leading=10, alignment=TA_CENTER))])
...
t4a_data.append([P_var('R<super>2</super>')] + [P_num(fs_stata[k][5]) for k in fs_keys])
...
t4b_data.append([P('', cell_left)] + [P(f'<i>PriceDelay</i>', ParagraphStyle('dv', fontName='Times-Italic',
...
story.append(Spacer(1, 4))
story.append(Paragraph(
    f"注：Panel A报告工具变量第一阶段回归结果（Stata reghdfe验证），Panel B报告第二阶段估计。"
