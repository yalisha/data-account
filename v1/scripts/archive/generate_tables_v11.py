"""
生成朱康(2025)会计研究格式的全部表格PDF — v12
v12变更:
  - 控制变量从15个改为13个（移除Analyst和Amihud，因为它们是机制变量）
  - 全部系数基于Stata reghdfe重跑结果更新
v11变更:
  - 表5机制: Analyst+Disp+Amihud三渠道, 同期+滞后双Panel
  - 表6异质性: SOE/Amihud/Size/Industry四维度, Panel A交互+Panel B分组
"""
...
controls = ['Size','Lev','ROA','TobinQ','Age','Growth','BoardSize','IndepRatio',
            'Dual','Top1Share','SOE','InstHold','AuditType']
...
rob_stata = {
    'indyear':  (-0.0028, 0.0006, '***', 43778, 0.472),  # (1) Ind*Year FE
    'provyear': (-0.0045, 0.0008, '***', 43842, 0.429),  # (2) Prov*Year FE
    'indadj':   (-0.0025, 0.0006, '***', 43843, 0.418),  # (3) Industry-year mean adj
    'no2024':   (-0.0052, 0.0010, '***', 38916, 0.428),  # (4) Drop 2024
    'noit':     (-0.0046, 0.0009, '***', 40720, 0.419),  # (5) Drop IT industry
    'mainboard':(-0.0032, 0.0013, '**',  18837, 0.412),  # (6) Main board only
    'psm':      (-0.0044, 0.0009, '***', 35228, 0.433),  # (7) PSM matched
    'twoway':   (-0.0046, 0.0012, '***', 43843, 0.419),  # (8) Two-way cluster SE
    'lead':     (-0.0052, 0.0012, '***', 38567, 0.429),  # (9) DU_kw_lead placebo: DU_kw coef when controlling for lead
}
...
# Override with new Stata results (13 controls)
iv_results['peer_iv'] = {'coef': -0.0143, 'se': 0.0046, 'p': 0.002, 'N': 43779, 'KP_F': 265.0}
iv_results['bartik_iv'] = {'coef': -0.0105, 'se': 0.0077, 'p': 0.173, 'N': 43058, 'KP_F': 60.4}
iv_results['lag_ols'] = {'coef': -0.0039, 'se': 0.0010, 'p': 0.0001, 'N': 37404}
iv_results['lag_iv'] = {'coef': -0.0157, 'se': 0.0052, 'p': 0.003, 'N': 37340, 'KP_F': 184.7}
iv_results['oster'] = {'delta_13': 76.3, 'delta_con': 76.3, 'beta_adj': 0.0043}
...
# ---- Mechanism models (v11: 同期+滞后, Analyst+Disp+Amihud) ----
print("  Mechanism models (v11: Analyst+Disp+Amihud, concurrent+lagged)...")
mech_dvs = ['Analyst', 'Disp', 'Amihud']
# Panel A: 同期 X_t → M_t
mech_results = {
    'Analyst': {"coef": 0.0273, "se": 0.0082, "sig": "***", "N": 43843, "R2": 0.793},
    'Disp':    {"coef": -0.0059, "se": 0.0015, "sig": "***", "N": 25092, "R2": 0.482},
    'Amihud':  {"coef": -0.0010, "se": 0.0003, "sig": "***", "N": 43843, "R2": 0.562},
}
# Panel B: 滞后一期 X_t → M_{t+1}
mech_lagged = {
    'Analyst': {"coef": 0.0335, "se": 0.0085, "sig": "***", "N": 37404, "R2": 0.816},
    'Disp':    {"coef": -0.0064, "se": 0.0018, "sig": "***", "N": 21160, "R2": 0.492},
    'Amihud':  {"coef": -0.0014, "se": 0.0003, "sig": "***", "N": 37404, "R2": 0.629},
}
...
pdf_path = f"{BASE}/results/v12_tables/regression_tables_v12.pdf"
...
fs_stata = {
    'peer':   ('DU_kw_peer', 0.5956, 0.0366, '***', 43779, None),
    'bartik': ('bartik_iv',  0.2867, 0.0369, '***', 43058, None),
    'lag':    ('peer_lag',   0.6013, 0.0442, '***', 37340, None),
}

# Second-stage: Stata reghdfe manual 2SLS (13 controls)
# Format: (coef, se, sig, N, R2)
iv_models_data = [
    (-0.004589, 0.000888, '***', 43843, 0.419),      # (1) OLS
    (-0.01430, 0.00460, '***', 43779, None),           # (2) Peer IV
    (-0.01050, 0.00770, '', 43058, None),              # (3) Bartik IV (n.s.)
    (-0.003900, 0.001000, '***', 37404, None),         # (4) Lag OLS
    (-0.01570, 0.00520, '***', 37340, None),           # (5) Lag IV
]

# KP F and DWH p from Stata (13 controls)
iv_kp_f = {2: 265.0, 3: 60.4, 5: 184.7}
iv_dwh_p = {2: 0.014, 3: 0.370, 5: 0.008}
...
# ============================================================
# 表5: 作用渠道检验 (v11: 6列, Analyst+Disp+Amihud, 同期+滞后)
# ============================================================
print("  Table 5...")

mech_dv_labels = ['Analyst', 'Disp', 'Amihud']

# --- 双Panel表格: 6列 (同期3列 + 滞后3列) ---
ncols = 6
t5_data = []
# 列号行
t5_data.append([P('', cell_left)] + [P(f'({i+1})', header_style) for i in range(ncols)])
# 被解释变量行
dv_s = ParagraphStyle('dv5', fontName='Times-Italic', fontSize=8, leading=10, alignment=TA_CENTER)
...
table5 = Table(t5_data, colWidths=[2.5*cm] + [2.4*cm]*ncols)
table5.setStyle(TableStyle([
    ...
    ('LINEBELOW', (0,7), (-1,7), 0.3, colors.black),  # Panel A底
...
story.append(Paragraph(
    ...
    "市场流动性改善价格发现效率（Amihud和Mendelson，1986；Chordia等，2008）。"
    "固定效应为企业+年份，标准误按行业×年份聚类。"
    "Analyst为分析师覆盖人数对数，Disp为分析师预测分歧度，"
    "Amihud为非流动性指标。Amihud回归中控制变量不包含Amihud本身。", note_style))
story.append(PageBreak())
