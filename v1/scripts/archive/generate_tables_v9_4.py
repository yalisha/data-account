"""
生成朱康(2025)会计研究格式的全部表格PDF — v9.4
v9.4变更:
  - 表3: 新增第9列DU_kw_lead反向因果排除(安慰剂检验)
  - 表5: 机制检验替换为Analyst/Disp/absDA三渠道, 含Panel A(X->M) + Panel B(Sobel) + Panel C(X+M->Y)
  - 表7: 改为附表(附表1)
  - 所有机制结果使用Stata reghdfe硬编码(reg_sample_v4.dta)
v9.3修复:
  - 表6 Panel A: 分组维度改为产权性质/机构持股/分析师覆盖(3维度6列)
  - 表6 Panel A: 组间差异检验改为Fisher permutation test p值(Stata验证)
v9.2修复:
...
pdf_path = f"{BASE}/results/v9_tables/regression_tables_v9_4.pdf"
...
t3_data.append([P('', cell_left)] + [P(f'<i>{dv}</i>', ParagraphStyle(f'dv3_{i}', fontName='Times-Italic',
...
story.append(Paragraph(
    ...
    "在匹配样本上以连续DU_kw回归。"
    "模型(8)使用企业和年份双向聚类标准误。"
    "模型(9)同时纳入DU_kw和DU_kw_lead(t+1期)，DU_kw_lead系数不显著(t=0.57)排除反向因果。", note_style))
story.append(PageBreak())
...
# --- Panel A: X -> M (第一阶段) ---
t5a_data = []
t5a_data.append([P('<b>Panel A: DU_kw -> M</b>', cell_left)] + [P('', cell_style)]*2)
t5a_data.append([P('', cell_left)] + [P(f'({i+1})', header_style) for i in range(3)])
t5a_data.append([P('', cell_left)] + [P(f'<i>{dv}</i>', ParagraphStyle(f'dv_{dv}', fontName='Times-Italic',
...
# --- Panel B: X + M -> Y (第二阶段) ---
t5b_data = []
t5b_data.append([P('<b>Panel B: DU_kw + M -> PriceDelay</b>', cell_left)] + [P('', cell_style)]*2)
t5b_data.append([P('', cell_left)] + [P(f'({i+4})', header_style) for i in range(3)])
t5b_data.append([P('', cell_left)] + [P(f'<i>PriceDelay</i>', ParagraphStyle(f'pd_{i}', fontName='Times-Italic',
