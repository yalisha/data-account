"""
生成朱康(2025)会计研究格式的全部表格PDF — v9
合并v8改进(表2变量标签、动态星号、表6非上标星号)到v7框架(保持N=43847样本)
"""
...
# ============================================================
print("Loading data...")
BASE = "/sessions/happy-eloquent-gauss/mnt/15会计研究"
...
# Fisher permutation test (stored + compute Size dimension)
...
# Compute Fisher test for Size dimension
print("  Running Fisher permutation test for Size...")
np.random.seed(42)
n_perm = 1000
obs_diff = het_results["大企业"]["coef"] - het_results["小企业"]["coef"]
perm_diffs = []
for _ in range(n_perm):
    perm_labels = np.random.permutation(reg['Size'].values >= med_size)
    sub1 = reg[perm_labels].copy()
    sub2 = reg[~perm_labels].copy()
    try:
        m1p = pf.feols(base_fml, data=sub1, vcov={"CRV1":"IndYear"})
        m2p = pf.feols(base_fml, data=sub2, vcov={"CRV1":"IndYear"})
        d = float(m1p.coef()['DU_kw']) - float(m2p.coef()['DU_kw'])
        perm_diffs.append(d)
    except:
        pass
fisher_p['企业规模'] = np.mean([abs(d) >= abs(obs_diff) for d in perm_diffs]) if perm_diffs else np.nan
...
pdf_path = f"{BASE}/results/v9_tables/regression_tables_v9.pdf"
