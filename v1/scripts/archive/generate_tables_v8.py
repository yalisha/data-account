"""
生成朱康(2025)会计研究格式的全部表格PDF — v8
修复：表2主变量标签、表4显著性星号动态化、表6 Fisher口径一致、窄列表格星号错位
"""
...
# 部分年份缺少行业映射，按公司前后填充以保持分组与聚类变量完整
...
reg = panel.dropna(subset=['PriceDelay','DU_kw'] + controls + ['IndYear']).copy()
...
for _, row in fisher_df.iterrows():
    if row['Dimension'] in fisher_dim_map:
        fisher_p[fisher_dim_map[row['Dimension']]] = row['p']
...
pdf_path = f"{BASE}/results/v8_tables/regression_tables_v8.pdf"
...
def P_coef(c, sig, dec=3, superscript=True):
    txt = f"{c:.{dec}f}"
    if sig:
        txt += f"<super>{sig}</super>" if superscript else sig
...
# Key variable rows: align variable labels with each model's actual regressor
key_rows = {
    'DU_kw':     [m0_main['DU_kw'], m1_coefs['DU_kw'], None, None, m4_main['DU_kw'], m5_main['DU_kw'], m6_main['DU_kw']],
    'DU_kw_ln':  [None, None, m2_main['DU_kw_ln'], None, None, None, None],
    'DU_sub_ln': [None, None, None, m3_main['DU_sub_ln'], None, None, None],
}
for var_name, col_vals in key_rows.items():
    cr = [P_var(var_name)]
    sr = [P('', cell_left)]
...
# Control variables: show in ALL models with controls (2)-(7)
# model order: m0(no ctrl), m1, m2, m3, m4, m5, m6
...
t2_data.append([P('Firm/Year', cell_left)] + [P(v, cell_style) for v in ['YES','YES','YES','YES','NO','YES','YES']])
t2_data.append([P('Ind/Year', cell_left)] + [P(v, cell_style) for v in ['NO','NO','NO','NO','YES','NO','NO']])
...
iv = iv_results
iv_models_data = [
    (iv['ols']['coef'], iv['ols']['se'], sig_stars(iv['ols'].get('p')), iv['ols']['N'], iv['ols']['R2']),
    (iv['peer_iv']['coef'], iv['peer_iv']['se'], sig_stars(iv['peer_iv'].get('p')), iv['peer_iv']['N'], None),
    (iv['bartik_iv']['coef'], iv['bartik_iv']['se'], sig_stars(iv['bartik_iv'].get('p')), iv['bartik_iv']['N'], None),
    (iv['lag_ols']['coef'], iv['lag_ols']['se'], sig_stars(iv['lag_ols'].get('p')), iv['lag_ols']['N'], None),
    (iv['lag_iv']['coef'], iv['lag_iv']['se'], sig_stars(iv['lag_iv'].get('p')), iv['lag_iv']['N'], None),
]
...
# ============================================================
# 表6: 异质性分析 (8列, landscape-style widths)
# ============================================================
print("  Table 6...")
...
t6_data = []
t6_data.append([P('', cell_left)] + [P_hdr(l) for l in het_labels])
t6_data.append([P('', cell_left)] + [P('<i>PriceDelay</i>', ParagraphStyle('dv', fontName='Times-Italic',
...
for label in ['Controls', 'Firm/Year']:
    t6_data.append([P(label, cell_left)] + [P('YES', cell_style)]*8)

t6_data.append([P_var('N')] + [P_int(het_results[g]['N']) for g in het_groups])
t6_data.append([P_var('R<super>2</super>')] + [P_num(het_results[g]['R2']) for g in het_groups])
...
table6 = Table(t6_data, colWidths=[2.2*cm] + [1.85*cm]*8)
...
story.append(Spacer(1, 4))
story.append(Paragraph(
    "注：组间差异P值基于Fisher组合检验（1000次随机抽样）。大/小企业按总资产对数中位数分组，"
    "高/低分析师覆盖按分析师跟踪人数中位数分组，高科技行业为证监会I类（信息技术）和M类（科学研究）。", note_style))
story.append(PageBreak())
